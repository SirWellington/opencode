...
Browsers are redirected to a sign-in page instead of the native password prompt. Check "Remember me" to stay signed in for 30 days, or leave it unchecked to end the session when the browser is closed.

| Method | Path        | Description                                                       |
| ------ | ----------- | ----------------------------------------------------------------- |
| `GET`  | `/sign-in`  | Show the sign-in page                                             |
| `POST` | `/sign-in`  | Sign in. Form fields: `username`, `password`, `remember`, `next`  |
| `POST` | `/sign-out` | End the session and clear the `opencode_session` cookie           |
